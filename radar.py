...
def procesiraj():
    ...
    for r in STACIONARNI_BAZA:
        vsi.append({"regija": "Radarbot Gold", "kraj": r["kraj"], "cas": "24/7", "lat": r["lat"], "lon": r["lon"], "vir": "Radarbot"})
    
    # 2. Waze (Vijolična)
    vsi.extend(skeniraj_waze())
    
    # 3. AMZS Radarji (Rumena)
    vsi.extend(skeniraj_splet("https://www.amzs.si/na-poti/stanje-na-cestah", ["radar", "merjenje"], "AMZS Radar", "AMZS"))
    
    # 4. DARS Zastoji (Rdeča)
    vsi.extend(skeniraj_splet("https://www.promet.si/sl/dogodki", ["zastoj", "nesreča", "zapora"], "DARS Promet", "DARS"))
    
    # 5. AMZS Kamere (Siva)
    vsi.extend(skeniraj_splet("https://www.amzs.si/na-poti/stanje-na-cestah", ["kamera", "pretočnost"], "AMZS Kamere", "Kamere"))

    # 6. Facebook (Modra)
    for id_s, ime in FB_SKUPINE.items():
        koord = dobi_koord(f"Radar {ime}")
        if koord and not any(abs(koord[0]-x['lat']) < 0.003 for x in vsi):
            vsi.append({"regija": ime, "kraj": f"FB Javljanje: {ime}", "cas": datetime.now().strftime("%H:%M"), "lat": koord[0], "lon": koord[1], "vir": "Facebook"})

    with open('radarji.json', 'w', encoding='utf-8') as f:
        json.dump(vsi, f, ensure_ascii=False, indent=4)
    print(f"Sistem posodobljen: Najdenih {len(vsi)} točk.")
